...
from src.config.constants import FILE_PATHS
...
class TestFileOperations:
    def test_load_files(self, data_service, tmp_path):
        ...
        with patch('src.services.data_service.FILE_PATHS', test_file_paths):
    ...
    def test_determine_filename(self, data_service):
        from src.ui.main_window import MainWindow
        window = MainWindow()
        
        # TRAY1 UC1 테스트
        assert window.determine_filename("TRAY 1", "UC-1") == FILE_PATHS["TRAY1_UC1"]
        
        # TRAY1 UC2 테스트
        assert window.determine_filename("TRAY 1", "UC-2") == FILE_PATHS["TRAY1_UC2"]
        
        # TRAY2 UC1 테스트
        assert window.determine_filename("TRAY 2", "UC-1") == FILE_PATHS["TRAY2_UC1"]
        
        # TRAY2 UC2 테스트
        assert window.determine_filename("TRAY 2", "UC-2") == FILE_PATHS["TRAY2_UC2"]
        
        # 알 수 없는 조합 테스트
        assert window.determine_filename("UNKNOWN", "UNKNOWN") == FILE_PATHS["UNKNOWN"]
